    ...
    <div className="mt-4 h-72 rounded-2xl border border-[#ddd4c7] bg-[#faf5ec] p-3">
      <svg viewBox="0 0 300 210" className="h-full w-full">
        ...
        {[0, 1, 2, 3, 4].map((step) => {
          ...
          const y = 40 + step * 35;
          return <text key={`y-${step}`} x="16" y={y + 3} textAnchor="end" fontSize="8.5" fill="#766352">{yTickFormatter(value)}</text>;
        })}
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-4 md:p-5">
        ...
        <MiniLineChart data={netWorthTrend} xKey="month" yKey="netWorth" stroke="#d68936" xAxisLabel="Month" yAxisLabel="Net Worth ($)" yTickFormatter={(v) => money(v)} />
      </Card>
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-4 md:p-5">
        ...
        <MiniLineChart data={collegeProjection} xKey="age" yKey="balance" stroke="#d68936" labelFormatter={(age) => `Age ${age}`} xAxisLabel="Kid's Age" yAxisLabel="Projected Balance ($)" yTickFormatter={(v) => money(v)} />
      </Card>
      ...
      <Card className="p-4 md:p-5">
        ...
        <MiniLineChart data={retirementProjection} xKey="age" yKey="balance" stroke="#d68936" labelFormatter={(age) => `${age}`} xAxisLabel="Cody & Ashley Age" yAxisLabel="Projected Balance ($)" yTickFormatter={(v) => money(v)} />
...
function GoalsView() {
  ...
  const loanPrincipal = Math.max(0, targetHome.price - downPayment);
  ...
  const totalMonthlyHousing = principalAndInterest + pmiMonthly + propertyTaxMonthly + hoaMonthly;
  const annualHousing = totalMonthlyHousing * 12;
  const preTaxIncomeAt80Net = (annualHousing * 100) / 80;
  const requiredGrossIncome = (preTaxIncomeAt80Net * 10) / 33;
