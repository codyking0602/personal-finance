    ...
    <div className="mt-4 h-72 rounded-2xl border border-[#ddd4c7] bg-[#faf5ec] p-3">
      <svg viewBox="0 0 300 210" className="h-full w-full">
        ...
        {[0, 1, 2, 3, 4].map((step) => {
          ...
          const y = 45 + step * 31;
          return <text key={`y-${step}`} x="31" y={y + 3} textAnchor="end" fontSize="8.5" fill="#766352">{yTickFormatter(value)}</text>;
        })}
        ...
        <text x="150" y="204" textAnchor="middle" fontSize="9" fill="#5e4b3b">{xAxisLabel}</text>
        <text x="6" y="108" textAnchor="middle" fontSize="9" fill="#5e4b3b" transform="rotate(-90 6 108)">{yAxisLabel}</text>
      </svg>
    </div>
  );
}
      ...
      <Card className="overflow-hidden p-0">
        ...
        <div className="p-4 md:p-5">
          ...
          <div className="mt-4 rounded-2xl bg-[#fff4e6] p-4">
            ...
            <div className="mt-1 text-sm text-[#6e5a47]">Required gross income (33% payment cap): <span className="font-black text-[#3f3025]">{money(requiredGrossIncome)}</span>/year</div>
          </div>
